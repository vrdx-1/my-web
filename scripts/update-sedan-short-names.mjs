...
const sedanPath = path.join(rootDir, 'data', 'category-models', 'sedan.json');
...
// Map: normalized alias -> English brand name (lowercase) เฉพาะรุ่นที่อยู่ใน sedan
...
for (const brand of brands) {
  ...
  for (const model of models) {
    ...
    if (!categoryIds.includes('sedan')) continue;
...
const sedanData = loadJson(sedanPath);
const modelNames = sedanData.modelNames ?? [];
...
sedanData.modelNames = updatedModelNames;
saveJson(sedanPath, sedanData);

console.log('Updated sedan.json: prepended English brand name for short/numeric model names (1–3 chars or all digits).');
