...
async function fetchClickRows(admin: AdminClient, startIso: string, nextYearStartIso: string) {
  ...
  for (;;) {
    ...
    const { data, error } = await admin
      .from('exchange_rate_popup_clicks')
      .select('created_at')
...
/**
 * GET /api/admin/daily-exchange-rate-popup-clicks?year=2026
 * คืนข้อมูลจำนวนการกดดูอัตราแลกเปลี่ยนโดยประมาณแบบรายวัน
 */
export async function GET(request: NextRequest) {
  ...
  const queryRes = await fetchClickRows(admin, startIso, nextYearStartIso);

  if (queryRes.error) {
    return internalServerError('admin/daily-exchange-rate-popup-clicks query failed', queryRes.error);
  }
  ...
  for (let i = 0; i < daysInYear; i++) {
    ...
    rows.push({ click_date: day, count: grouped.get(day) ?? 0 });
  }
  ...
  return NextResponse.json({
    rows,
    summary: { year, todayCount, totalInYear, daysWithData },
  });
}
