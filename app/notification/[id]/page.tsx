...
export default function NotificationDetail() {
  ...
  const fetchInteractions = useCallback(async (tab: string) => {
    setListLoading(true);
    const tableName = tab === 'likes' ? 'post_likes' : tab === 'saves' ? 'post_saves' : 'post_shares';
    const { data } = await supabase.from(tableName).select(`user_id, profiles:user_id (*)`).eq('post_id', id);
    if (data) setUserList(data.map((item: any) => item.profiles).filter(p => p !== null));
  ...
  const getOnlineStatus = (lastSeen: string | null) => {
    ...
    return { isOnline: false, text: `ອອນລາຍລ່າສຸດ ${Math.floor(diffInHours / 24)} ມື้ທີ່ແລ້ວ` };
  };
  ...
  if (loading || !post) return <div style={{ textAlign: 'center', padding: '50px' }}>ກຳລັງໂຫຼດ...</div>;
    ...
    <main style={{ maxWidth: '600px', margin: '0 auto', background: '#fff', minHeight: '100vh', position: 'relative', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ position: 'sticky', top: '53px', background: '#fff', zIndex: 90 }}>
        <div style={{ display: 'flex', borderBottom: '1px solid #ddd' }}>
          {[{ key: 'likes', icon: '❤️', c: post.likes }, { key: 'saves', icon: '🔖', c: post.saves }, { key: 'shares', icon: '🔗', c: post.shares }].map((t) => (
      ...
      <div style={{ paddingBottom: '100px' }}>
        {listLoading ? <div style={{ textAlign: 'center', padding: '20px' }}>ກຳລັງໂຫຼດ...</div> : 
          userList.map((user, idx) => (
            <div key={idx} style={{ padding: '12px 15px', display: 'flex', alignItems: 'center', gap: '12px', borderBottom: '1px solid #f0f0f0' }}>
              <img src={user.avatar_url} style={{ width: '45px', height: '45px', borderRadius: '50%', objectFit: 'cover' }} />
      ...
      {/* 4. Viewing Mode (ปรับ UI ให้ตรงตามหน้า Home เป๊ะๆ) */}
      {viewingPost && (() => {
      ...
      {/* 5. Full Screen Mode (Swipe + บันทึกภาพ) */}
      {fullScreenImages && (
