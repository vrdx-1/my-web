...
export default function SavedPosts() {
 ...
 const fetchSavedPosts = async (userIdOrToken: string, isInitial = false) => {
 setLoadingMore(true);
 ...
 *,
 ...
 .filter(p => {
 if (!p) return false;
 // เช็คสถานะ Tab (พร้อมขาย/ขายแล้ว)
 if (p.status !== tab) return false;
 
 // เงื่อนไข Shadow Hide: 
 // 1. ถ้าไม่ได้ถูกซ่อน (is_hidden เป็น false หรือ null) ให้แสดงปกติ
 // 2. ถ้าถูกซ่อน (is_hidden เป็น true) จะแสดงให้เห็นเฉพาะเจ้าของโพสต์เท่านั้น
 ...
 const newSavedMap = { ...savedPosts };
 formattedPosts.forEach(p => newSavedMap[p.id] = true);
 setSavedPosts(newSavedMap);
 ...
 const { data } = await supabase.from('post_likes').select('post_id').eq('user_id', userIdOrToken);
 if (data) {
 ...
 const idOrToken = currentSession ? currentSession.user.id : getPrimaryGuestToken();
 
 setPage(0);
 setHasMore(true);
 fetchSavedPosts(idOrToken, true);
 fetchLikedStatus(idOrToken);
 ...
 // โหลดเพิ่มเมื่อเลื่อนหน้าจอ (ทำงานเมื่อ page เปลี่ยน)
 useEffect(() => {
 if (page > 0) {
 const idOrToken = session ? session.user.id : getPrimaryGuestToken();
 fetchSavedPosts(idOrToken);
 }
 }, [page]);
 ...
 return myGuestPosts.some(p => p.post_id === post.id);
 };
 ...
 <h1 style={{ fontSize: '18px', fontWeight: 'bold', margin: 0 }}>ລາຍການທີ່ບັນທຶກ</h1>
 </div>
 ...
 <div onClick={() => toggleSave(post.id)} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center' }}><svg width="22" height="22" viewBox="0 0 24 24" className="animate-pop" fill="#FFD700" stroke="#FFD700" strokeWidth="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg><span style={{ fontSize: '14px', fontWeight: '600', color: '#FFD700', marginLeft: '4px' }}>{post.saves || 0}</span></div>
