...
export function SavedComparePostsContent() {
  const router = useRouter();
  const { session, activeProfileId, authUserId } = useSessionAndProfile();
  const [savedSource, setSavedSource] = React.useState<SavedSource | null | undefined>(undefined);
  ...
  React.useEffect(() => {
    ...
    if (isValidKey(userIdCandidate) && UUID_PATTERN.test(userIdCandidate)) {
      setSavedSource({
        table: 'post_saves',
        column: 'user_id',
  ...
  const fetchSavedPosts = React.useCallback(async () => {
    ...
    const nextPostIds = savesData
      .map((item) => String(item.post_id || ''))
      .filter((postId) => isValidKey(postId));
    ...
    const order = new Map(nextPostIds.map((postId, index) => [postId, index]));
  ...
  const loaded = savedSource !== undefined;
  const showEmpty = loaded && !loadingPosts && posts.length === 0;
  const showLoadingSkeleton = !loaded || (loadingPosts && posts.length === 0);

  const handleRemoveSavedPost = React.useCallback(async (postId: string) => {
  ...
  }, [fetchSavedPosts, savedSource]);

  const handleClearAll = React.useCallback(async () => {
    if (!savedSource || count === 0) return;
        ...
        {posts.map((post) => (
          <ComparePostRow
            ...
            onRemove={() => {
              ...
              void handleRemoveSavedPost(nextPostId);
            }}
          />
        ))}
      </div>
