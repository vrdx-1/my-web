...
export default function AdminShareClicksPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/admin/daily-share-clicks?year=${selectedYear}`, {
          credentials: 'include',
        });

        if (!res.ok) {
          const payload = await res.json().catch(() => ({}));
          throw new Error(payload?.error || 'Failed to load share click stats');
        }
  ...
  return (
    <main style={{ maxWidth: '600px', margin: '0 auto', background: '#ffffff', minHeight: '100vh', fontFamily: LAO_FONT, paddingBottom: '40px' }}>
      ...
      <div style={{ textAlign: 'center', padding: '40px 20px', color: '#777', fontSize: '14px' }}>
        ຕົວເລກນີ້ເປັນຈຳນວນຄັ້ງທີ່ Guest ແລະ User ທົ່ວໄປກົດແຊໂພສ
      </div>
    </main>
  );
}
