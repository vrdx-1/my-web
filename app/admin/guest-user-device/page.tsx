...
export default function AdminGuestUserDevicePage() {
  const [loading, setLoading] = useState(true);
  const [guestList, setGuestList] = useState<GuestRow[]>([]);
  const [deviceByVisitorId, setDeviceByVisitorId] = useState<Record<string, DeviceInfo>>({});
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // 1) ดึง user_sessions ที่ user_id เป็น null (Guest) → ได้ visitor_id ล่าสุดต่อ guest
      ...
      (sessionsData || []).forEach((row: { visitor_id: string }) => {
        const vid = String(row.visitor_id ?? '').trim();
        if (vid && !seen.has(vid)) {
          seen.add(vid);
          uniqueVisitorIds.push(vid);
        }
      });

      setGuestList(uniqueVisitorIds.map((visitor_id) => ({ visitor_id })));
      ...
      if (uniqueVisitorIds.length > 0) {
        const BATCH = 100;
        for (let i = 0; i < uniqueVisitorIds.length; i += BATCH) {
          const batch = uniqueVisitorIds.slice(i, i + BATCH);
      ...
      if (Object.keys(visitorToAgent).length === 0 && uniqueVisitorIds.length > 0) {
        ...
        if (!fallbackErr && fallbackLogs && Array.isArray(fallbackLogs)) {
          const vidSet = new Set(uniqueVisitorIds.map((id) => String(id).trim()));
      ...
      uniqueVisitorIds.forEach((vid) => {
        ...
        const userAgent = visitorToAgent[v] ?? visitorToAgent[vid] ?? null;
        const deviceName = parseDeviceName(userAgent);
        deviceMap[v] = { deviceName };
      });
      setDeviceByVisitorId(deviceMap);
    } catch (err) {
      console.error('Fetch guest user device error:', err);
      setGuestList([]);
      setDeviceByVisitorId({});
    } finally {
      setLoading(false);
    }
  };
              ...
              {guestList.length === 0 ? (
                <tr>
                  <td colSpan={4} style={{ padding: '24px', color: '#6b7280', textAlign: 'center' }}>
                    ບໍ່ມີຂໍ້ມູນ Guest User
                  </td>
                </tr>
              ) : (
                guestList.map((row, index) => {
                  const vid = String(row.visitor_id).trim();
                  const device = deviceByVisitorId[vid] ?? deviceByVisitorId[row.visitor_id];
                  ...
                  return (
                    <tr key={row.visitor_id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                      ...
                      <td style={{ padding: '12px 8px', color: '#1a1a1a', fontWeight: '500', verticalAlign: 'middle', fontFamily: 'monospace', fontSize: '13px' }}>
                        Guest {maskVisitorId(row.visitor_id)}
                      </td>
