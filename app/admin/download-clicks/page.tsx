...
export default function AdminDownloadClicksPage() {
  ...
  const [allRows, setAllRows] = useState<DailyDownloadClickRow[]>([]);
  const [summary, setSummary] = useState<DownloadClickSummary>({
    year: currentYear,
    todayCount: 0,
    totalInYear: 0,
    daysWithData: 0,
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/admin/daily-download-clicks?year=${selectedYear}`, {
          credentials: 'include',
        });

        if (!res.ok) {
          const payload = await res.json().catch(() => ({}));
          throw new Error(payload?.error || 'Failed to load download click stats');
        }
