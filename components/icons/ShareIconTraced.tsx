...
export type ShareIconTracedProps = React.ImgHTMLAttributes<HTMLImageElement> & {
  size?: number;
  /** สี fill (ค่าเริ่มต้นเท่า like/save/view เดิม) */
  fillColor?: string;
};

/** สีเดียวกับ like, save, view */
const ACTION_FILL = '#4a4d52';

/**
 * Share icon – ลูกศรแชร์ (share arrow, filled)
 */
export function ShareIconTraced({ size = 24, width, height, style, fillColor, ...props }: ShareIconTracedProps) {
  const w = width ?? size;
  const h = height ?? size;
  const fill = fillColor ?? ACTION_FILL;
  return (
    <svg
      width={w}
      height={h}
